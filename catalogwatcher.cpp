...
namespace CatalogManage
{
    CatalogWatcher::CatalogWatcher(QObject *p, const fs::path &f_d, const fs::path &t_d):QObject(p), from_directory(f_d), to_directory(t_d)
    {
        ...
        fw->addPath( QString::fromUtf8(from_directory.string().c_str()));
        allFiles=files(from_directory);
        copyF(from_directory, to_directory);
        connect(fw, QFileSystemWatcher::directoryChanged, [this](){  std::this_thread::sleep_for(std::chrono::milliseconds(100)); catalogChanged();});
    ...
    void CatalogWatcher::copyF(const fs::path &from_dir, const fs::path &to_dir)
    {
    ...
    void CatalogWatcher::catalogChanged()
    {
